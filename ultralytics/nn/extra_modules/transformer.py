...
__all__ = ['TransformerEncoderLayer_LocalWindowAttention', 'AIFI_LPE', 'TransformerEncoderLayer_DAttention', 'TransformerEncoderLayer_HiLo', 
           ...
           'AIFI_SEFFN', 'TransformerEncoderLayer_Pola_SEFFN_Mona_DyT', 'AIFI_EDFFN', 'DL_AFIM', 'TransformerEncoderLayer_MSLA',
...
class Attention_EPGO(nn.Module):
    ...
    def forward(self, x):
        ...
        # 检查 gate 输出
        if torch.isnan(gate_out).any() or torch.isinf(gate_out).any():
            # 验证模式下的 fallback：使用固定比例的 k
            dynamic_k = max(1, N // 2)  # 使用 50% 的注意力
            print(f"[WARNING] Gate 输出 NaN (可能是 BN 统计损坏)，使用默认 k={dynamic_k}")
        else:
            gate_mean = gate_out.view(B, -1).mean()
            # 额外安全检查
            if torch.isnan(gate_mean):
                dynamic_k = max(1, N // 2)
            else:
                dynamic_k = int(N * gate_mean.item())
                dynamic_k = max(1, min(dynamic_k, N))
